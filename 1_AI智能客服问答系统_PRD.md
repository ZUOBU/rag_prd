# 1. 产品定位
**银行AI智能客服与问答系统** 是基于大语言模型（LLM）与检索增强生成（RAG）技术构建的新一代智能问答中枢。
- **对内**：面向行内业务员、柜员、客服坐席等内部员工，新增内部信息知识检索渠道，使业务规章问询更便捷、回答更完整，提升检索效率与回答质量。解决新员工“问询难”的问题，减少新员工对老员工的依赖，显著降低企业培训与内部沟通成本。
- **对外 [规划至二期]**：升级现有App、小程序、网上银行等端口中的客服系统，实现从“关键词匹配”到“语义理解”的跨越，提升回答质量与客户自助解决率，分流人工客服压力，降低客服坐席整体工作量。
...
当下人工智能发展迅速， 大语言模型（LLM）的爆发及RAG（检索增强生成）技术的成熟，为金融领域解决“语义理解”与“知识溯源”提供了技术方案，使得构建一个既懂业务又能通俗回答的智能系统成为可能。
头部产品支付宝、蚂蚁财富等已上线基于AI技术的问答系统，银行中招行已上线AI问答系统。其他银行如江苏银行等也在部署相关系统。
...
- 赋能员工知识库（对内）
  - 实现个人业务知识的语义检索与自动总结，提升问题命中率、解决新员工“问询难”的问题，降低培训成本。
...
- 提升对客服务效能（对外）
  - 将客户问题的语义识别准确率提升至 90% 以上。
  - 显著提升客户自助解决率，预计降低人工坐席分流压力 30%-40%。
- 降本增效与闭环管理
  - 通过RAG系统减少人工维护成千上万条FAQ的成本。
  - 利用一、二期的反馈机制（Badcase收集），建立数据驱动的知识进化闭环，持续优化模型表现。
...
- **数据层**：零售业务条线文档库、现存QA问答库、Badcase库、行为日志库。
- **核心算法层（RAG核心）**：文档解析器、文本分块（Chunking）、Embedding向量化、混合检索引擎（BM25+Vector）、重排序（Reranking）。
- **模型服务层**：意图识别大模型、生成式大模型（LLM）、安全合规审核模型。
- **应用层**：员工端问答页面、常见问题快捷入口、后台运营管理平台。
...
sequenceDiagram
    ...
    participant Doc as 知识库(重排)
    participant LLM as 大语言模型
    User->>App: 1.点击常见问题 / 2.输入Query
    alt 是点击首屏常见问题配置问题
        App->>QA: 发送问题ID
        QA-->>App: 直接返回预设答案
    else 是输入自由Query
        App->>LLM: 意图识别/改写
        LLM-->>App: 解析出查询词
        App->>QA: 优先在QA库进行混合检索匹配
        App->>Doc: 在知识库中进行混合检索
        QA-->>Doc: 汇总召回Top K
        Doc->>Doc: 过Rerank模型进行交叉重排序
        Doc->>LLM: 将Top N Chunk 及原问题拼装进Prompt
        LLM-->>App: 流式生成答案
    end
    App-->>User: 展示结果(含溯源元数据参考)
    alt 回答不佳
        User->>App: 点踩并反馈原因(一期必填原因)
        App->>App: 落库进Badcase管理台
    end
```
...
4. **多轮对话判断**
   - **时间轮次阈值判断**：若当前提问距该用户的上一轮回复时间间隔在 **5分钟（300秒）以内**，即认为未超时，属于“多轮连续对话语境”。因具有极大概率的跟问属性，一律纳入触发改写。
